...
// Aggiungi questo codice in app.js, subito DOPO la funzione renderSottoassiemi()

// Setup filtro sottoassiemi
function setupSottoassiemiFilter() {
    ...
    // Evento input con debouncing per performance
    let debounceTimer;
...
function filterSottoassiemi(query) {
    ...
    if (!searchTerm) {
        ...
        cards.forEach(card => {
            card.classList.remove('filtered-hidden', 'filtering');
            visibleCount++;
        });
    } else {
        // Applica filtro
    ...
    // Highlight input se nessun risultato
...
// Aggiorna le statistiche del filtro
function updateFilterStats(visible = null) {
    ...
    if (visibleCountEl) visibleCountEl.textContent = visibleNum;
    if (totalCountEl) totalCountEl.textContent = total;
}

// MODIFICA la funzione renderSottoassiemi esistente aggiungendo alla fine:
function renderSottoassiemi() {
    sottoassiemiContainer.innerHTML = '';
...
// MODIFICA il submit del form commessa aggiungendo setupSottoassiemiFilter():
// Cerca questa parte nel commessaForm.addEventListener('submit', ...)
// e aggiungi alla fine (dopo setupCSVModal()):
...
function setupCSVModal() {
    ...
    const btnConfirmImportCSV = document.getElementById('btnConfirmImportCSV'); // ← CAMBIATO

    // ✅ AGGIUNGI QUESTO CONTROLLO
    if (!btnImportCSV || !csvModal || !btnConfirmImportCSV) {
        console.warn('⚠️ Elementi CSV modal non trovati, setup posticipato');
        return;
    }
    ...
    // Conferma import
    btnConfirmImportCSV.addEventListener('click', () => { // ← CAMBIATO
...
function renderCSVPreview(data, validCount, errorCount, warningCount) {
    ...
    // Abilita/disabilita bottone import
    const btnImport = document.getElementById('btnConfirmImportCSV');  // ✅ ID CORRETTO
    if (errorCount > 0) {
